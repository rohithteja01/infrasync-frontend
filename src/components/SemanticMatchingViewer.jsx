...
import { BrainCircuit, CheckCircle2, HelpCircle, XCircle, RefreshCw, AlertCircle, FileSpreadsheet, Filter, Sliders } from 'lucide-react';
...
export default function SemanticMatchingViewer({ executionFile: propExecFile, scheduleFile: propSchedFile, projectContext } = {}) {
  ...
  useEffect(() => {
    if (projectContext?.is_fallback && projectContext?.activities) {
      const semanticCandidates = projectContext.activities.filter(a => a.match_tier === 'SEMANTIC' || a.match_tier === 'DISCOVERED');
      setSemanticData({
        execution_filename: initialExec || 'demo_fallback_data',
        schedule_filename: initialSched || 'baseline_schedule.xlsx',
        unresolved_execution_count: semanticCandidates.length,
        possible_semantic_matches: 1,
        no_match_count: semanticCandidates.length - 1,
        matches: semanticCandidates.map(a => {
          const isSemantic = a.match_tier === 'SEMANTIC';
          return {
            execution_activity_id: a.activity_id,
            execution_activity_name: a.activity_name,
            discipline: a.discipline,
            level: a.activity_id.includes('L5') ? 'L5' : 'L6',
            match_status: isSemantic ? 'possible_match' : 'unmatched',
            similarity_score: isSemantic ? 0.68 : 0.32,
            threshold_applied: threshold,
            best_match: isSemantic ? {
  ...
  const fetchSemanticMatches = useCallback(async (execFile, schedFile, thresh) => {
    if (hasNoData || !execFile || projectContext?.is_fallback) return;
    setLoading(true);
    setError(null);
    try {
      const url = `/api/ingestion/match/semantic?execution_file=${encodeURIComponent(execFile)}&schedule_file=${encodeURIComponent(schedFile)}&threshold=${thresh}`;
      ...
      setError(err.message || 'Failed to compute semantic activity matches.');
  ...
  useEffect(() => {
    fetchSemanticMatches(executionFile, scheduleFile, threshold);
  }, [fetchSemanticMatches, executionFile, scheduleFile, threshold]);
      ...
      <div className="bg-white dark:bg-[#181818] border border-slate-200 dark:border-[#2e2e2e] rounded-xl p-8 text-center shadow-xs">
        <div className="max-w-md mx-auto space-y-3">
          ...
          <p className="text-sm text-slate-500 dark:text-neutral-400">
            Upload a project data file in Project Intelligence to inspect vector/semantic activity matching.
          </p>
        </div>
      </div>
    );
  ...
  const matches = semanticData?.matches || [];
  ...
  const unresolvedCount = semanticData?.unresolved_execution_count ?? 0;
  const possibleSemanticMatches = semanticData?.possible_semantic_matches ?? 0;
  const noMatchCount = semanticData?.no_match_count ?? 0;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h3 className="text-base font-bold text-slate-900">Context / Semantic Matching</h3>
                <span className="text-[11px] bg-teal-50 text-teal-800 border border-teal-200 px-2 py-0.5 rounded font-mono font-semibold">
                  Feature 2.9
                </span>
              </div>
              <p className="text-xs text-slate-500 mt-0.5">
                Local TF-IDF & cosine similarity on activity context (name, description, discipline, WBS) for unresolved activities
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center space-x-1.5">
              <label htmlFor="sem-exec-file" className="text-xs font-medium text-slate-500">
                Execution:
              </label>
              <select
                id="sem-exec-file"
                ...
                disabled={loading}
                className="text-xs font-medium text-slate-800 bg-slate-50 border border-slate-300 rounded px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-teal-500"
              >
                {execOptions.map((f) => (
            ...
            <div className="flex items-center space-x-1.5">
              <label htmlFor="sem-sched-file" className="text-xs font-medium text-slate-500">
                Schedule:
              </label>
              <select
                id="sem-sched-file"
            ...
            <div className="flex items-center space-x-1.5 bg-slate-50 border border-slate-200 px-2 py-1 rounded">
              ...
              <label htmlFor="threshold-select-sem" className="text-xs font-medium text-slate-500">
                Threshold:
              </label>
              <select
                id="threshold-select-sem"
                value={threshold}
            ...
            <button
              onClick={() => fetchSemanticMatches(executionFile, scheduleFile, threshold)}
              disabled={loading}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {/* Unresolved Execution Activities */}
        ...
        <div className="bg-white border border-teal-100 rounded-lg p-4 shadow-sm bg-gradient-to-br from-white to-teal-50/30">
          <div className="flex items-center justify-between">
            ...
            <div className="p-1.5 bg-teal-50 rounded text-teal-600 border border-teal-200">
              <BrainCircuit className="w-4 h-4" />
            </div>
          </div>
          <div className="text-2xl font-bold text-teal-700 mt-2 font-mono">
            {loading ? '—' : possibleSemanticMatches}
          </div>
          <div className="text-[11px] text-teal-700/80 mt-1">
            Similarity &ge; {Math.round(threshold * 100)}% (Pending planner review)
          </div>
        </div>
      ...
      {error && (
        <div className="p-4 bg-rose-50 border border-rose-200 rounded-md flex items-start space-x-3 text-xs text-rose-700">
          ...
          <div>
            <p className="font-semibold text-rose-900">Semantic Matching Notice</p>
            <p className="mt-0.5">{error}</p>
          </div>
        </div>
      )}

      {/* Semantic Matching Table Container */}
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/70">
          <div>
            <h3 className="text-sm font-semibold text-slate-900">
              Context & Semantic Matching Register
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              High-context text representation similarity for unresolved execution items
            </p>
          </div>
          ...
          <div className="flex items-center space-x-1 bg-slate-100 p-1 rounded-md border border-slate-200 text-xs">
            ...
            >
              Possible ({possibleSemanticMatches})
            </button>
            <button
        ...
        <div className="p-6">
          {loading ? (
            <div className="p-12 text-center text-slate-500 text-xs flex flex-col items-center justify-center space-y-2">
              ...
              <span>Vectorizing and calculating contextual cosine similarity...</span>
            </div>
            ...
            <div className="border border-slate-200 rounded-lg overflow-hidden shadow-2xs">
                ...
                <table className="w-full text-left text-xs text-slate-700">
                  ...
                  <tbody className="divide-y divide-slate-200 bg-white">
                    {filteredMatches.map((m, idx) => {
                      ...
                      return (
                        <tr key={idx} className={`transition-colors ${isPossible ? 'hover:bg-teal-50/30' : 'hover:bg-slate-50/50'}`}>
                          ...
                          <td className="px-3.5 py-2.5 text-center border-r border-slate-100 whitespace-nowrap">
                            <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-semibold border ${getDisciplineBadge(m.execution_discipline)}`}>
                              {m.execution_discipline || 'General'}
                            </span>
                          </td>
                          <td className="px-3.5 py-2.5 border-r border-slate-100 whitespace-nowrap">
                            {m.suggested_schedule_activity_id ? (
                              <span className="font-mono font-semibold text-teal-900 bg-teal-50 px-1.5 py-0.5 rounded border border-teal-200">
                                {m.suggested_schedule_activity_id}
                              </span>
                            ) : (
                          ...
                          <td className="px-3.5 py-2.5 text-center border-r border-slate-100 whitespace-nowrap">
                            <span className={`font-mono font-bold text-xs ${isPossible ? 'text-teal-700' : 'text-slate-500'}`}>
              ...
              <div className="px-4 py-2.5 bg-slate-50 border-t border-slate-200 text-xs text-slate-500 flex flex-col sm:flex-row items-center justify-between gap-2">
                <span>
                  Showing <strong>{filteredMatches.length}</strong> of <strong>{matches.length}</strong> unresolved activities evaluated against <strong>{scheduleFile}</strong>
                </span>
                <span className="font-mono text-[11px] text-slate-400">
                  Context / Semantic Matching • Feature 2.9
                </span>
              </div>
            </div>
          ) : (
            <div className="p-8 border border-dashed border-slate-300 rounded-lg text-center text-slate-500 text-xs bg-slate-50/50">
              No semantic candidate activities match the current filter ({filter}).
            </div>
          )}
        </div>
      </div>
